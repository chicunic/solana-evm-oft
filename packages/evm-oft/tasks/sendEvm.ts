...
// Minimal typed view over the IOFT ABI; ethers v5 Contract method calls are untyped otherwise
interface IOFTContract extends Contract {
...
// Minimal typed view over the ERC20 ABI
...
// Structural view of the LayerZero config module; eid is number to avoid cross-package enum identity issues
interface LayerZeroConfig {
  contracts: { contract: { eid: number; contractName?: string | null; address?: string | null } }[];
}
...
export async function sendEvm(
  ...
  if (oftAddress) {
    ...
    const configModule = (await import(path.resolve("./", oappConfig))) as {
      default: LayerZeroConfig | (() => LayerZeroConfig | Promise<LayerZeroConfig>);
    };
    const layerZeroConfig = configModule.default;
  ...
  // now attach
  const oft = (await srcEidHre.ethers.getContractAt(ioftArtifact.abi, wrapperAddress, signer)) as IOFTContract;

  // 3️⃣ fetch the underlying ERC-20
  const underlying = await oft.token();

  // 4️⃣ fetch decimals from the underlying token
  const erc20 = (await srcEidHre.ethers.getContractAt("ERC20", underlying, signer)) as ERC20Contract;
