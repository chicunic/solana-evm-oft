...
export async function getBlockExplorerLink(srcEid: number, txHash: string): Promise<string | undefined> {
  ...
  if (!res.ok) return;
  // Value may be absent for unknown networks, so the record is explicitly partial
  const all = (await res.json()) as Record<string, { blockExplorers?: { url: string }[] } | undefined>;
