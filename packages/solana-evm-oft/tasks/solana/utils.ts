...
export const findSolanaEndpointIdInGraph = async (
  ...
  try {
    ...
    } satisfies SubtaskLoadConfigTaskArgs)) as OAppOmniGraph;
  } catch (error) {
    if (error instanceof Error) {
      throw new Error(`Failed to load OApp configuration: ${error.message}`, { cause: error });
    } else {
      throw new Error("Failed to load OApp configuration: Unknown error", { cause: error });
    }
  }
  ...
  for (const { vector } of graph.connections) {
    ...
    // TS cannot see assignments made inside checkSolanaEndpoint, so widen the narrowed type
    const foundEid = solanaEid as EndpointId | null;
    if (foundEid !== null) return foundEid;
  }
 ...
 */
export function silenceSolana429(_connection: Connection): void {
