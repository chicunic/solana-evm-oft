...
task("lz:oft:solana:debug", "Manages OFTStore and OAppRegistry information")
  ...
  .setAction(async (taskArgs: DebugTaskArgs) => {
    ...
    const printChecks = () => {
      ...
      DebugLogger.keyValue("Token Mint Authority is OFT Store", unwrapOption(mintAccount.mintAuthority) === oftStore);
      DebugLogger.separator();
    };
    ...
    const printPeerConfigs = async () => {
      ...
      for (let index = 0; index < dstEids.length; index++) {
        ...
        const info = peerConfigInfos.at(index);
        ...
        if (info) {
          ...
          DebugLogger.keyValue("Send", decodeLzReceiveOptions(uint8ArrayToHex(info.enforcedOptions.send, true)), 2);
          DebugLogger.keyValue(
            "SendAndCall",
    ...
    if (action) {
      switch (action) {
        ...
        case DEBUG_ACTIONS.GET_PEERS:
          ...
          break;
        default:
          console.error(`Invalid action specified. Use any of ${Object.keys(DEBUG_ACTIONS).join(",")}.`);
      }
    } else {
      printOftStore();
      printDelegate();
...
function printOAppReceiveConfigs(
  ...
  for (let i = 0; i < oAppReceiveConfig.length; i++) {
    ...
    if (typeof item === "object") {
      ...
      DebugLogger.keyValue(oAppReceiveConfigIndexesToKeys[i], item, 2);
    }
  }
}
...
function printOAppSendConfigs(oAppSendConfig: Awaited<ReturnType<typeof getSolanaSendConfig>>, peerChainName: string) {
  ...
  for (let i = 0; i < oAppSendConfig.length; i++) {
    ...
    if (typeof item === "object") {
      ...
      DebugLogger.keyValue(sendOappConfigIndexesToKeys[i], item, 2);
    }
  }
}
