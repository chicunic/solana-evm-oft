  ...
  .setAction(async (args: MasterArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    if (args.oftAddress || args.oftProgramId) {
      DebugLogger.printWarning(
        ...
        `For network: ${endpointIdToNetwork(args.srcEid)}, OFT: ${String(args.oftAddress) + (args.oftProgramId ? `, OFT program: ${args.oftProgramId}` : "")}`,
      );
    }

    // route to the correct function based on the chain type
    if (chainType === ChainType.EVM) {
      result = await sendEvm(args, hre);
    } else if (chainType === ChainType.SOLANA) {
      result = await sendSolana(args);
    } else {
