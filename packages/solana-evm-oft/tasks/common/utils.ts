...
// Minimal shape of a deployment metadata entry used by getBlockExplorerLink
interface DeploymentMetadataEntry {
...
export async function getBlockExplorerLink(srcEid: number, txHash: string): Promise<string | undefined> {
  ...
  if (!res.ok) return;
  const all = (await res.json()) as Record<string, DeploymentMetadataEntry | undefined>;
  const meta = all[network];
  const explorer = meta?.blockExplorers?.at(0)?.url;
  if (explorer) {
