...
// Tuple matching the SendParam struct expected by the OFT quoteSend/send functions
type SendParamTuple = [number, Uint8Array, BigNumber, BigNumber, string, string, string];

// Minimal typed surface of the EndpointV2Mock contract used in this test
interface EndpointV2MockContract extends Contract {
  setDestLzEndpoint(destination: string, lzEndpoint: string): Promise<ContractTransaction>;
}

// Minimal typed surface of the MyOFTMock contract used in this test
interface MyOFTContract extends Contract {
  connect(signer: SignerWithAddress): MyOFTContract;
  ...
  quoteSend(sendParam: SendParamTuple, payInLzToken: boolean): Promise<[BigNumber, BigNumber]>;
  send(
    sendParam: SendParamTuple,
...
describe("MyOFT Test", function () {
  ...
  let myOFTA: MyOFTContract;
  let myOFTB: MyOFTContract;
  ...
  before(async function () {
    ...
    const myOFTFactory: unknown = await ethers.getContractFactory("MyOFTMock");
    MyOFT = myOFTFactory as ContractFactory;
  ...
  beforeEach(async function () {
    ...
    myOFTA = (await MyOFT.deploy("aOFT", "aOFT", mockEndpointV2A.address, ownerA.address)) as MyOFTContract;
    myOFTB = (await MyOFT.deploy("bOFT", "bOFT", mockEndpointV2B.address, ownerB.address)) as MyOFTContract;
  ...
  it("should send a token from A address to B address via each OFT", async function () {
    ...
    const sendParam: SendParamTuple = [
      eidB,
