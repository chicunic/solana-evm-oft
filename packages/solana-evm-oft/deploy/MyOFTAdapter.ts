...
// Shape of the optional oftAdapter entry expected on the network config in hardhat.config.ts
...
const deploy: DeployFunction = async (hre) => {
  ...
  const { oftAdapter } = hre.network.config as { oftAdapter?: OftAdapterConfig };
  if (oftAdapter == null) {
  ...
  const { address } = await deployments.deploy(contractName, {
    from: deployer,
    args: [
      oftAdapter.tokenAddress, // token address
